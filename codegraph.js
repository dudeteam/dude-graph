...
cg.Block = (function () {
    ...
    Block.prototype.addPoint = function (cgPoint) {
        if (cgPoint.cgBlock !== this) {
            throw new Error("Block::addPoint() Point is not bound to this block: `" + cgPoint.cgName + "`");
        }
        if (cgPoint.isOutput && this.outputByName(cgPoint.cgName) || !cgPoint.isOutput && this.inputByName(cgPoint.cgName)) {
            throw new Error("Block::addPoint() Block has already an " +
...
cg.Connection = (function () {
    ...
    var Connection = pandora.class_("Connection", function (cgOutputPoint, cgInputPoint) {
        ...
        (function Initialization() {
            if (!cgOutputPoint.isOutput) {
                throw new Error("Connection() cgOutputPoint is not an output");
            }
            if (cgInputPoint.isOutput) {
                throw new Error("Connection() cgInputPoint is not an input");
            }
        })();
    ...
    Connection.prototype.otherPoint = function (cgPoint) {
        ...
        throw new Error("Connection::otherPoint() Point `" + cgPoint.cgName + "` is not in this connection");
    };
...
cg.Renderer = (function () {
    ...
    /**
     * Registers a cg.Block with a custom creator/updater/position getter
     * @param {cg.Block} blockType
     */
    Renderer.prototype.addBlockType = function (blockType) {
        var typeName = pandora.typename(blockType.prototype);
        if (blockType.rendererBlockCreator !== undefined) {
            this._rendererBlockCreators[typeName] = blockType.rendererBlockCreator;
        }
        if (blockType.rendererBlockUpdater !== undefined) {
            this._rendererBlockUpdaters[typeName] = blockType.rendererBlockUpdater;
        }
        if (blockType.pointPositionGetter !== undefined) {
            this._pointPositionGetters[typeName] = blockType.pointPositionGetter;
        }
        this._cgGraph.loader.addBlockType(blockType);
    };
 ...
 * @private
 */
cg.Renderer.prototype._getRendererPointPosition = function (rendererPoint, relative) {
    ...
    var func = this._pointPositionGetters[pandora.typename(rendererPoint.rendererBlock.cgBlock)];
    if (func) {
