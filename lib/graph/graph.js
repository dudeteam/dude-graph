...
dudeGraph.Graph = function () {
     ...
     * @type {Object<dudeGraph.Graph.graphTypeNameTypedef, dudeGraph.Graph.graphTypeDataTypedef>}
     * @private
     */
    this._graphTypes = this._graphTypes = {
        ...
        "String": {
            ...
            "typeCompatibles": ["Number", "Boolean"]
        },
        "Boolean": {
            "typeConvert": function (value) {
 ...
 * @param {dudeGraph.Graph.graphTypeDataTypedef} graphType
 */
dudeGraph.Graph.prototype.addGraphType = function (graphType) {
    if (!_.isUndefined(this._graphTypes[graphType.typeName])) {
        throw new Error("`" + this.graphFancyName + "` cannot redefine value type`" + graphType.typeName + "`");
    }
    this._graphTypes[graphType.typeName] = graphType;
};
//noinspection JSUnusedGlobalSymbols
/**
 * Removes the type from the graph
 * @param {dudeGraph.Graph.graphTypeNameTypedef} typeName
 */
dudeGraph.Graph.prototype.removeGraphType = function (typeName) {
    throw new Error("`" + this.graphFancyName + "` removeGraphType(`" + typeName + "`) not implemented");
};
/**
 * Returns the graphType for the given typeName, or null
 * @param {dudeGraph.Graph.graphTypeNameTypedef} typeName
 * @returns {dudeGraph.Graph.graphTypeDataTypedef|null}
 */
dudeGraph.Graph.prototype.graphTypeByName = function (typeName) {
    return this._graphTypes[typeName] || null;
};

/**
 * Converts the given value to the given type if possible or returns undefined
 * @param {dudeGraph.Graph.graphTypeNameTypedef} valueType
...
dudeGraph.Graph.prototype.convertValue = function (valueType, value) {
    ...
    var graphType = this.graphTypeByName(valueType);
    if (graphType === null) {
        throw new Error("`" + this.graphFancyName + "` has no graph type `" + valueType + "`");
    }
    if (_.isUndefined(graphType.typeConvert)) {
        // TODO: throw message
        throw this._graphTypes;
    }
    return graphType.typeConvert(value);
};
/**
...
dudeGraph.Graph.prototype.convertConnection = function (outputPoint, inputPoint) {
    var typeInfo = this._graphTypes[inputPoint.pointValueType];
    ...
    return outputPoint.pointValueType === inputPoint.pointValueType ||
        (outputPoint.pointCanConvert && inputPoint.pointCanConvert &&
        _.includes(typeInfo.typeCompatibles, outputPoint.pointValueType));
};
...
dudeGraph.Graph.prototype.cloneBlocks = function (blocks) {
    throw new Error("`" + this.graphFancyName + "` cloneBlocks(`" + blocks + "`) not implemented");
};
/**
...
/**
 * Returns the next available blockId (blockIds are not consecutive)
 * @returns {String}
 */
...
dudeGraph.Graph.prototype.variableModels = function () {
    return _.map(this._graphVariables, function (graphVariable) {
        return {
            "item": {
                "name": graphVariable.variableName,
                "icon": "fa fa-tag",
                "data": {
...
/**
 * @typedef {String} dudeGraph.Graph.graphTypeNameTypedef
 */

/**
 * @typedef {Object} dudeGraph.Graph.graphTypeDataTypedef
 ...
 * @property {*} pointValue
 * @property {String} [pointCanConvert]
 * @property {String} [pointSingleConnection]
 */
