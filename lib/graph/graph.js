 ...
 */
dudeGraph.Graph.prototype.blockCustom = function (blockType) {
 ...
 */
dudeGraph.Graph.prototype.pointCustom = function (pointType) {
...
dudeGraph.Graph.prototype.removeType = function (type) {
    throw new Error("`" + this.graphFancyName + "` removeType(`" + type + "`) not implemented");
};
/**
...
dudeGraph.Graph.prototype.convertConnection = function (outputValueType, inputValueType) {
    var typeInfo = this._graphTypes[inputValueType];
    if (_.isUndefined(typeInfo)) {
    ...
    return outputValueType === inputValueType || _.includes(typeInfo.compatible, outputValueType);
};
        ...
        _.forEach(recursiveModels, function (model) {
            if (!_.isUndefined(model.item)) {
                ...
                var eligibleModelPoints = _.filter(modelPoints, function (modelPoint) {
                    return point.pointOutput && graph.convertConnection(point.pointValueType, modelPoint.pointValueType) ||
                        !point.pointOutput && graph.convertConnection(modelPoint.pointValueType, point.pointValueType);
                });
                if (!_.isEmpty(eligibleModelPoints)) {
                    _.forEach(eligibleModelPoints, function (eligibleModelPoint) {
                        eligibleModels.push(_.merge(model, {
                            "item": {
