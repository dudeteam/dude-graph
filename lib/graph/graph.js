...
dudeGraph.Graph = function () {
     ...
     * @type {Object<String, {converter: dudeGraph.Graph~convertType, compatible: Array<String>}>}
     * @private
     */
 ...
 * @param {dudeGraph.Graph~convertType} [typeInfo.convert=_.noop]
...
dudeGraph.Graph.prototype.convertValue = function (valueType, value) {
    ...
    if (_.isUndefined(typeInfo)) {
        throw new Error("Cannot find converter to convert `" + value + "` to type `" + valueType + "`");
    }
...
dudeGraph.Graph.prototype.convertConnection = function (outputValueType, inputValueType) {
    ...
    if (_.isUndefined(typeInfo)) {
        throw new Error("Cannot find compatible type to convert connection from `" +
            outputValueType + "` to `" + inputValueType + "`");
    }
    return inputValueType === outputValueType || _.includes(typeInfo.compatible, outputValueType);
};
 ...
 * @callback dudeGraph.Graph~convertType
