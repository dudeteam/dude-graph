...
dudeGraph.Graph.prototype.convertConnection = function (outputPoint, inputPoint) {
    ...
    if (typeof outputPoint.acceptConnect != "undefined" && !outputPoint.acceptConnect(inputPoint)) {
        if (this._graphErrno !== null && this._graphErrno !== previousErrno) {
            this.errno(new Error("`" + outputPoint.pointFancyName +
                "` cannot accept to connect to `" + inputPoint.pointFancyName + "`: " + this._graphErrno.message));
        } else {
            this.errno(new Error("`" + outputPoint.pointFancyName +
                "` cannot accept to connect to `" + inputPoint.pointFancyName + "`"));
        }
        return false;
    }
...
dudeGraph.Graph.prototype.addBlock = function (block) {
    ...
    _.forOwn(block.blockTemplates, function (template, templateName) {
...
dudeGraph.Graph.prototype.connectPoints = function (outputPoint, inputPoint) {
    if (outputPoint.pointBlock === null) {
        throw new Error("`" + outputPoint.pointFancyName + "` cannot connect to another point when not bound to a block");
    }
    if (inputPoint.pointBlock === null) {
        throw new Error("`" + inputPoint.pointFancyName + "` cannot connect to another point when not bound to a block");
    }
    ...
    if (connectionFound !== null) {
        throw new Error("`" + connectionFound.connectionFancyName + "` already exists");
    }
...
dudeGraph.Graph.prototype.disconnectPoints = function (outputPoint, inputPoint) {
    if (outputPoint.pointBlock === null) {
        throw new Error("`" + outputPoint.pointFancyName + "` cannot disconnect from another point when not bound to a block");
    }
    if (inputPoint.pointBlock === null) {
        throw new Error("`" + inputPoint.pointFancyName + "` cannot disconnect from another point when not bound to a block");
    }
    var connectionFound = _.find(this._graphConnections, function (connection) {
            return connection.connectionOutputPoint === outputPoint && connection.connectionInputPoint === inputPoint;
        }) || null;
 ...
 * @private
 */
dudeGraph.Graph.prototype._addConnection = function (connection) {
    if (typeof _.find(connection.connectionOutputPoint.pointConnections, connection) !== "undefined") {
        throw new Error("`" + connection.connectionOutputPoint.pointFancyName + "` cannot redefine `" + connection.connectionFancyName + "`");
    }
    if (typeof _.find(connection.connectionInputPoint.pointConnections, connection) !== "undefined") {
        throw new Error("`" + connection.connectionInputPoint.pointFancyName + "` cannot redefine `" + connection.connectionFancyName + "`");
    }
    connection.connectionOutputPoint.pointConnections.push(connection);
    connection.connectionInputPoint.pointConnections.push(connection);
 ...
 * @private
 */
dudeGraph.Graph.prototype._removeConnection = function (connection) {
    if (typeof _.find(connection.connectionOutputPoint.pointConnections, connection) == "undefined") {
        throw new Error("`" + connection.connectionOutputPoint.pointFancyName + "` has no connection `" + connection.connectionFancyName + "`");
    }
    if (typeof _.find(connection.connectionInputPoint.pointConnections, connection) === "undefined") {
        throw new Error("`" + connection.connectionInputPoint.pointFancyName + "` has no connection `" + connection.connectionFancyName + "`");
    }
    _.pull(connection.connectionOutputPoint.pointConnections, connection);
    _.pull(connection.connectionInputPoint.pointConnections, connection);
    _.pull(this._graphConnections, connection);
};
                    ...
                    "blockOutputs": [
                        {
                            ...
                            "pointName": "value",
                            "pointPolicy": ["MULTIPLE_CONNECTIONS", "CONVERSION"]
                        }
                    ]
                }
            }
