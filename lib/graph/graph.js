 ...
 */
dudeGraph.Graph.prototype.addType = function (type, converter) {
    if (!_.isUndefined(this._graphTypes[type])) {
        throw new Error("Grpah has already a type ` " + type + "`");
    }
    this._graphTypes[type] = converter;
};
/**
...
dudeGraph.Graph.prototype.convertValue = function (valueType, value) {
    ...
    var converter = this._graphTypes[valueType];
    if (_.isUndefined(converter)) {
        throw new Error("Cannot find suitable converter to convert `" + value + "` for type `" + valueType + "`");
    }
