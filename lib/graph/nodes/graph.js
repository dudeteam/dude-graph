cg.Graph = (function () {
    ...
    Graph.prototype.addEntity = function (entity, parent) {
        if (parent.graph !== this) {
            ...
            if (entity instanceof cg.Graph) {
                this.emit("error", new cg.GraphError("Can't nest a graph in a graph", entity));
                return null;
            }
            if (entity instanceof cg.Group) {
                if (this._registeredGroupIds[entity._id]) {
                    this.emit("error", new cg.GraphError("This id is already used for group {0}", entity._id));
                    return null;
                }
                this._registeredGroupIds[entity._id] = entity;
            } else if (entity instanceof cg.Block) {
                if (this._registeredBlockIds[entity._id]) {
                    this.emit("error", new cg.GraphError("This id is already used for a block {0}", entity._id));
                    return null;
                }
    ...
    Graph.prototype._remove = function () {
        this.emit("error", new cg.GraphError("Can't remove graph entity.", parent));
    };
