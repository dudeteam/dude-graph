...
dudeGraph.AssignationBlock.prototype.validatePoints = function () {
    ...
    if (!(this.inputByName("this") instanceof dudeGraph.Point)) {
        throw new Error("AssignationBlock `" + this.blockFancyName + "` must have an input `this` of type `Point`");
    }
    if (!(this.inputByName("other") instanceof dudeGraph.Point)) {
        throw new Error("AssignationBlock `" + this.blockFancyName + "` must have an input `other` of type `Point`");
    }
    if (this.inputByName("this").pointValueType !== this.inputByName("other").pointValueType) {
        throw new Error("AssignationBlock `" + this.blockFancyName + "` inputs `this` and `other` must have the same pointValueType");
    }
