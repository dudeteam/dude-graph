...
/**
 * Tries to update the points types from templates parameters to match the `point` type with the given `type`.
...
dudeGraph.Block.prototype.updateTemplate = function (cgPoint, type) {
    if (cgPoint.cgTemplate === null) {
        return;
    }
    var templatePoints = _.filter(this._cgOutputs.concat(this._cgInputs), function (templatePoint) {
        return templatePoint.cgTemplate === cgPoint.cgTemplate;
    });
    var sameTemplate = _.every(templatePoints, function (templatePoint) {
        return templatePoint.cgValueType === type;
    });
    if (!sameTemplate) {
        _.forEach(templatePoints, function (templatePoint) {
            if (templatePoint.cgValueType !== type && templatePoint.cgConnections.length > 0) {
                throw new Error("Cannot update templated type to `" + type + "`: `" + templatePoint.cgName +
                    "` is connected to a `" + templatePoint.cgValueType + "`");
            }
        });
    }
    _.forEach(templatePoints, function (templatePoint) {
        templatePoint.cgValueType = type;
    });
};
