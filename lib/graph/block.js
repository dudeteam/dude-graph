 ...
 * @param {Object<String, dudeGraph.Graph~template>} [blockData.blockTemplates=null]
 * @class
...
dudeGraph.Block.prototype.updateTemplate = function (templateName, valueType, ignoreEmit) {
    ...
    try {
        _.forEach(this._blockOutputPoints, function (point) {
            if (point.pointTemplate === templateName) {
                point.changeValueType(valueType, ignoreEmit);
            }
        });
        _.forEach(this._blockInputPoints, function (point) {
            if (point.pointTemplate === templateName) {
                point.changeValueType(valueType, ignoreEmit);
            }
        });
...
dudeGraph.Block.prototype.removePoint = function (point) {
    ...
    if (point.pointOutput) {
        _.pull(this._blockOutputPoints, point);
    }
    if (!point.pointOutput) {
        _.pull(this._blockInputPoints, point);
    }
