...
dudeGraph.Point = function (cgBlock, data, isOutput, pointType) {
    ...
    Object.defineProperty(this, "cgValueType", {
        ...
        set: function (cgValueType) {
            if (cgValueType !== null && this._cgTemplate !== null && this._cgBlock.cgTemplates[this._cgTemplate].indexOf(cgValueType) === -1) {
                throw new Error("Cannot change valueType of point " + this._cgName + ": `" +
                    cgValueType + "` is not in the list of possible templates: `[" + this._cgBlock.cgTemplates[this._cgTemplate].join(", ") + "]`");
            }
            var oldValueType = this._cgValueType;
            this._cgValueType = cgValueType;
            if (!_.isUndefined(oldValueType)) {
                this._cgGraph.emit("cg-point-value-type-change", this, oldValueType, cgValueType);
            }
        }.bind(this)
    });
    this._cgValueType = undefined;
    this.cgValueType = data.cgValueType;
    if (_.isUndefined(this._cgValueType)) {
    ...
    this._cgValue = _.isUndefined(data.cgValue) ? null : data.cgValue;
};
