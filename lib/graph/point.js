...
dudeGraph.Point.prototype.initialize = function (pointData) {
    ...
    if (!_.isString(this._pointName)) {
        throw new Error("`pointName` must be a non-null String");
    }
    if (!_.isString(this._pointValueType)) {
        throw new Error("`pointValueType` must be a non-null String");
    }
};
...
dudeGraph.Point.prototype.changeValueType = function (valueType, ignoreEmit) {
    if (!this._pointBlock.blockGraph.hasType(valueType)) {
        throw new Error("Unknown type `" + this._pointValueType + "`");
    }
    var assignValue = this._pointBlock.blockGraph.convertValue(valueType, this._pointValue);
    if (_.isUndefined(assignValue)) {
        throw new Error("`" + this._pointValue + "` is not compatible with type `" + valueType + "`");
    }
    var oldValueType = this._pointValueType;
    this._pointValueType = valueType;
    if (!ignoreEmit) {
        this._pointBlock.blockGraph.emit("value-type-change", valueType, oldValueType);
    }
    this.changeValue(assignValue, !!ignoreEmit);
};
/**
...
dudeGraph.Point.prototype.changeValue = function (value, ignoreEmit) {
    ...
    if (!ignoreEmit) {
        this._pointBlock.blockGraph.emit("value-change", assignValue, oldValue);
    }
};
