/**
 * Point is a named value that is held by a block
 * It can accepts a direct value or be connected to other compatible block points
...
dudeGraph.Point.prototype.initialize = function (pointData) {
    ...
    if (this.hasPolicy(dudeGraph.PointPolicy.SINGLE_CONNECTION, dudeGraph.PointPolicy.MULTIPLE_CONNECTIONS)) {
        throw new Error("`" + this.pointFancyName + "` `pointPolicy` cannot mix " +
            "`SINGLE_CONNECTION` and `MULTIPLE_CONNECTIONS`");
    }
};

/**
 * Called when this point is added to the block
 */
dudeGraph.Point.prototype.added = function () {};
/**
 * Called when this point is removed from the block
 */
dudeGraph.Point.prototype.removed = function () {};
 ...
 * @returns {Boolean}
 */
dudeGraph.Point.prototype.hasPolicy = function () {
    var point = this;
    return _.every(arguments, function (argument) {
       return dudeGraph.PointPolicy.has(point.pointPolicy, argument);
    });
};
/**
 ...
 * @returns {Boolean}
 */
dudeGraph.Point.prototype.acceptConnect = function (otherPoint) {
    return otherPoint === otherPoint;
};
/**
 * Returns whether this point is connected to the given point
 * @param {dudeGraph.Point} otherPoint
 */
dudeGraph.Point.prototype.connectedTo = function (otherPoint) {
    var point = this;
    return _.some(this._pointConnections, function (connection) {
        return connection.other(point) === otherPoint;
    });
};
 ...
 */
dudeGraph.Point.prototype.connect = function (otherPoint) {
    if (this._pointOutput) {
        return this._pointBlock.blockGraph.connectPoints(this, otherPoint);
    }
    return this._pointBlock.blockGraph.connectPoints(otherPoint, this);
};
/**
 * Disconnects from the given point
 * @param {dudeGraph.Point} otherPoint
 */
dudeGraph.Point.prototype.disconnect = function (otherPoint) {
    if (this._pointOutput) {
        return this._pointBlock.blockGraph.disconnectPoints(this, otherPoint);
    }
    return this._pointBlock.blockGraph.disconnectPoints(otherPoint, this);
};
/**
