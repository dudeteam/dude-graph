cg.Point = (function () {
    ...
    Point.prototype.addConnection = function (connection) {
        ...
        if (this.action.graph.hasConnection()) {
    ...
    Point.prototype.removeConnection = function (connection) {
        ...
        if (this.action.graph.hasConnection(connection)) {
            this.action.graph.removeConnection(connection);
        }
        var point = connection.otherPoint(this);
        if (point.hasConnection(connection)) {
     ...
     */
    Point.prototype.removeConnections = function () {
        for (var i = 0; i < this._connections.length; ++i) {
            this.removeConnection(this._connections[i]);
        }
    };
