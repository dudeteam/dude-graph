...
cg.Renderer.prototype._renderBlock = function (block) {
    ...
    block.on("reorder", this._reorderBlock.bind(this, block, blockGroup));
    block.on("remove", function() { blockGroup.remove(); });
    this._updateBlock(block, blockGroup);
    this._reorderBlock(block, blockGroup);
...
cg.Renderer.prototype._renderBlockPicker = function (picker, element) { // TODO resize the picker when value change
    ...
    picker.on("change", function (value) {
        ...
        pickerElement = this[functionName](picker, element);
        width = this._grid.round(pickerElement.data("width") + this._config.block["padding"] * 2 + this._config.point["circle-size"]);
...
cg.Renderer.prototype._renderPickerBoolean = function (picker, element) {
    ...
    //var checkbox = this.checkbox(element, picker.value);
    //checkbox.data("width", 10);
    //checkbox.transform("T " + [this._config.block["padding"] / 2, this._config.getter["height"] / 2 - 5]);
    //return checkbox;
};

/**
 * Reorder block element.
 * @param block {cg.Block}
 * @param blockGroup {Element}
 * @private
 */
cg.Renderer.prototype._reorderBlock = function (block, blockGroup) {
    blockGroup.appendTo(this._blockLayer);
};
 ...
 * @private
 */
cg.Renderer.prototype._updateBlock = function (block, blockGroup) {
    blockGroup.transform("T" + block.absolutePosition.toArray());
};