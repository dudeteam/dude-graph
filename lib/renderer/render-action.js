...
cg.Renderer.prototype._renderAction = function (action) {
    if (action.model.type === "getter") {
        return this._renderGetter(action);
    }
...
cg.Renderer.prototype._renderGetter = function (getter) {
    ...
    getter.on("move", this._updateAction.bind(this, getter, getterGroup));
    getter.on("remove", function() { getterGroup.remove(); });
    this._updateAction(getter, getterGroup);
