...
cg.Renderer.prototype._renderPoint = function (point, element) {
    ...
    pointGroup.mousedown(function (e) {
        this._cursorPoint = {
            isInput: !point.isInput,
            position: new cg.Vec2(e.x, e.y),
            type: point.type
        };
...
cg.Renderer.prototype._getPointRelativePosition = function (point) {
    return new cg.Vec2(
        (point.isInput ? this._style.action["padding"] : point.action.data.computedWidth - this._style.action["padding"]),
...
cg.Renderer.prototype._getPointAbsolutePosition = function (point) {
    ...
    return new cg.Vec2(
        actionPosition.x + (point.isInput ? this._style.action["padding"] : point.action.data.computedWidth - this._style.action["padding"]),
