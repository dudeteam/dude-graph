 ...
 * @private
 */
cg.Renderer.prototype._setNode = function (node, element, key) {
    key = key || 'node';
...
cg.Renderer.prototype._dragNode = function (node, nodeGroup) {
    ...
    nodeGroup.drag(
        function move(dx, dy) {
            ...
            nodeGroup.data('cg.origin.delta').copy(new cg.Vec2(dx, dy));
            // Move the node (apply zoom)
            node.position.add(new cg.Vec2(this._zoom.get(ddx), this._zoom.get(ddy)));
        ...
        function start(x, y, e) {
            ...
            if (node instanceof cg.Block) {
                ...
                sendChildrenToFront(nodeGroup);
                // Unparentize
                unparentize(nodeGroup);
            }
            // Cancel mouse propagation
            cg.preventCallback(e);
            // Save the previous delta movement
            nodeGroup.data('cg.origin.delta', new cg.Vec2());
            // Selection
            if (!e.shiftKey) {
        ...
        function end() {
            ...
            for (var i = dropElements.length - 1; i >= 0; --i) {
                ...
                var dropGroup = this._getNode(dropElements[i]);
