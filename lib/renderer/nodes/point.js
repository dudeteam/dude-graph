...
dudeGraph.RenderPoint.prototype.dragConnectionEvent = function () {
    ...
    renderPoint._d3Circle.call(d3.behavior.drag()
        .on("dragstart", function () {
            ...
            fakePoint.pointAbsolutePosition = d3.mouse(renderPoint._renderer._d3Root.node());
            if (renderPoint._point.isOutput) {
                renderConnection = new dudeGraph.RenderConnection(renderPoint._renderer, null, renderPoint, fakePoint);
            } else {
                renderConnection = new dudeGraph.RenderConnection(renderPoint._renderer, null, fakePoint, renderPoint);
            }
        ...
        .on("dragend", function () {
            ...
            if (renderPointFound) {
                try {
                    ...
                    var stuff = null;
                    if (renderPoint._point.isOutput) {
                        stuff = new dudeGraph.RenderConnection(renderPoint._renderer, connection, renderPoint, renderPointFound);
                    } else {
                        stuff = new dudeGraph.RenderConnection(renderPoint._renderer, connection, renderPointFound, renderPoint);
                    }
                    renderPoint._renderer._renderConnections.push(stuff);
                    renderPoint._renderer.createD3Connections();
                } catch (ex) {
