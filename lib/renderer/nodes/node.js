    ...
    this._d3Node.call(d3.behavior.drag()
        ...
        .on("drag", function () {
            var translateRenderNode = function (currentRenderNode) {
                if (renderNode._renderer._config.grid.activated) {
                    currentRenderNode.movePosition[0] += d3.event.dx;
                    currentRenderNode.movePosition[1] += d3.event.dy;
                    currentRenderNode.nodePosition[0] = Math.round(currentRenderNode.movePosition[0] / renderNode._renderer._config.grid.spacing) * renderNode._renderer._config.grid.spacing;
                    currentRenderNode.nodePosition[1] = Math.round(currentRenderNode.movePosition[1] / renderNode._renderer._config.grid.spacing) * renderNode._renderer._config.grid.spacing;
                } else {
