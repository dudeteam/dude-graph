...
dudeGraph.RenderNode.prototype.select = function () {
    this._d3Folie = this._d3Node.append("svg:polygon").attr("fill", "pink");
...
/**
 * Applies dom events on this block in order to select and move it.
 */
dudeGraph.RenderNode.prototype.moveEvent = function () {
    ...
    this._d3Node.call(d3.behavior.drag()
        ...
        .on("drag", function () {
            renderNode.nodePosition[0] += d3.event.dx;
            renderNode.nodePosition[1] += d3.event.dy;
            renderNode.move();
            var currentNode = renderNode.nodeParent;
            while (currentNode !== null) {
                currentNode.computePosition();
                currentNode.computeSize();
                currentNode.update();
                currentNode = currentNode.nodeParent;
            }
            // TODO: Private API
            _.forEach(renderNode._renderer._renderConnections, function (renderConnection) {
                if (renderConnection.outputRenderPoint.renderBlock === renderNode ||
                    renderConnection.inputRenderPoint.renderBlock === renderNode) {
