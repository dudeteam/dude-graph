...
dudeGraph.RenderBlock = function (renderer, rendererBlockId, block, renderPanel) {
    ...
    Object.defineProperty(this, "renderGroupParent", {
        ...
        set: function (renderGroupParent) {
            if (this._renderGroupParent !== null) {
                this._renderGroupParent.removeChild(this);
            }
            if (renderGroupParent !== null) {
                renderGroupParent.addChild(this);
            }
            this._renderGroupParent = renderGroupParent;
        }.bind(this)
    });
 ...
 * @override
 */
dudeGraph.RenderBlock.prototype.create = function (d3Block, forceUpdate) {
    dudeGraph.RenderNode.prototype.create.call(this, d3Block);
    var color = this._renderer.config.blockColors[this._block.blockName] || this._renderer.config.blockColors.default;
    var textColor = this._renderer.contrastTextColor(color);
    this._d3Rect = this._d3Node.append("svg:rect")
        .classed("dude-graph-block-wrapper", true)
        .attr({
            "rx": this._renderer.config.block.borderRadius,
            "ry": this._renderer.config.block.borderRadius,
            "fill": color
        });
    this._d3Title = this._d3Node.append("svg:text")
        .classed("dude-graph-block-title", true)
        .classed("dude-graph-block-title-light", textColor === "white")
        .classed("dude-graph-block-title-dark", textColor === "black")
