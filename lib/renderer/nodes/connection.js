...
dudeGraph.RenderConnection = function (renderer, connection, outputRenderPoint, inputRenderPoint) {
    ...
    Object.defineProperty(this, "connectionId", {
        get: function () {
            return this.outputRenderPoint.renderBlock.nodeId + ":" + this.outputRenderPoint.point.pointName + "," +
                this.inputRenderPoint.renderBlock.nodeId + ":" + this.inputRenderPoint.point.pointName;
        }.bind(this)
    });
...
dudeGraph.RenderConnection.prototype.update = function () {
    ...
    var inputColor = this._renderer.config.typeColors[this.inputRenderPoint.point.pointValueType];
    var outputColor = this._renderer.config.typeColors[this.outputRenderPoint.point.pointValueType];
    this._d3Connection
        .attr({
            "d": this._d3Line(connectionPoints),
            "stroke": inputColor || outputColor || this._renderer.config.typeColors.default,
 ...
 * @param {Number?} renderConnectionData.connectionIndex
...
dudeGraph.RenderConnection.buildRenderConnection = function (renderer, renderConnectionData) {
    var connection = renderConnectionData.connection || renderer._graph.graphConnections[renderConnectionData.connectionIndex];
    if (!connection) {
        throw new Error("Connection at index `" + renderConnectionData.connectionIndex + "` does not exist");
    }
    ...
    if (outputRenderBlock === null) {
        throw new Error("Connection at index `" + renderConnectionData.connectionIndex +
    ...
    if (outputRenderBlock.block !== connection.connectionOutputPoint.pointBlock) {
        throw new Error("Connection at index `" + renderConnectionData.connectionIndex +
            "`: output render block `" + outputRenderBlock.nodeFancyName +
            "` is not holding a reference to the output block `" + connection.connectionOutputPoint.pointBlock.blockId + "`");
    }
    if (inputRenderBlock.block !== connection.connectionInputPoint.pointBlock) {
        throw new Error("Connection at index `" + renderConnectionData.connectionIndex +
            "`: input render block `" + inputRenderBlock.nodeFancyName +
            "` is not holding a reference to the input block `" + connection.connectionInputPoint.pointBlock.blockId + "`");
    }
    var outputRendererPoint = renderer.renderPointByName(outputRenderBlock, connection.connectionOutputPoint.pointName, true);
    var inputRendererPoint = renderer.renderPointByName(inputRenderBlock, connection.connectionInputPoint.pointName, false);
    if (!outputRendererPoint) {
        throw new Error("Connection at index `" + renderConnectionData.connectionIndex +
            "`: Cannot find outputRenderPoint `" + connection.connectionOutputPoint.pointName + "`");
    }
    if (!inputRendererPoint) {
        throw new Error("Connection at index `" + renderConnectionData.connectionIndex +
            "`: Cannot find inputRenderPoint `" + connection.connectionInputPoint.pointName + "`");
    }
