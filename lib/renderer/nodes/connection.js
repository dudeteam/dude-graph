...
dudeGraph.RenderConnection = function (renderer, connection, outputRenderPoint, inputRenderPoint) {
    ...
    /**
     * Whether this connection is a dragging connection
     * @type {Boolean}
     * @private
     */
...
dudeGraph.RenderConnection.prototype.update = function () {
    var inputColor = this._renderer._config.typeColors[this.inputRenderPoint.point.cgValueType];
    var outputColor = this._renderer._config.typeColors[this.outputRenderPoint.point.cgValueType];
    this._d3Connection
        .attr("d", this.computePath())
        .attr("stroke", inputColor || outputColor);
    if (this._dragging) {
        this._d3Connection
...
dudeGraph.RenderConnection.prototype.computePath = function () {
    ...
    if (this._outputRenderPoint.pointAbsolutePosition[0] < this._inputRenderPoint.pointAbsolutePosition[0]) {
        step += Math.max(-100, this._outputRenderPoint.pointAbsolutePosition[0] - this._inputRenderPoint.pointAbsolutePosition[0]);
    }
    return "M" + this._outputRenderPoint.pointAbsolutePosition[0] + "," +
        (this._outputRenderPoint.pointAbsolutePosition[1]) + "C" +
        (this._outputRenderPoint.pointAbsolutePosition[0] + step) + "," +
        (this._outputRenderPoint.pointAbsolutePosition[1]) + " " +
        (this._inputRenderPoint.pointAbsolutePosition[0] - step) + "," +
        (this._inputRenderPoint.pointAbsolutePosition[1]) + " " +
        (this._inputRenderPoint.pointAbsolutePosition[0]) + "," +
        (this._inputRenderPoint.pointAbsolutePosition[1]);
    /*
...
dudeGraph.RenderConnection.buildRenderConnection = function (renderer, renderConnectionData) {
    ...
    var outputRenderBlock = renderer.renderBlockById(renderConnectionData.outputRendererBlockId);
    var inputRenderBlock = renderer.renderBlockById(renderConnectionData.inputRendererBlockId);
