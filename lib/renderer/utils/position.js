...
dudeGraph.Renderer.prototype._getRendererNodesBoundingBox = function (rendererNodes) {
    ...
    _.forEach(rendererNodes, function (rendererNode) {
        if (!topLeft) {
            topLeft = rendererNode.position;
        }
        if (!bottomRight) {
            bottomRight = [rendererNode.position[0] + rendererNode.size[0], rendererNode.position[1] + rendererNode.size[1]];
        }
        topLeft[0] = Math.min(rendererNode.position[0], topLeft[0]);
        topLeft[1] = Math.min(rendererNode.position[1], topLeft[1]);
...
dudeGraph.Renderer.prototype._computeRendererBlockSize = function (rendererBlock) {
    ...
    var maxOutput = 0;
    var maxInput = 0;
    rendererBlock.size[0] = blockSize[0] + this._config.block.padding * 2;
