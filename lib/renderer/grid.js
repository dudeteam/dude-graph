cg.Renderer.Grid = (function () {
     ...
     * @constructor
     */
    var Grid = pandora.class_("Grid", function (paper, config) {
        this._config = config || {
            "pattern-size": 10,
            "step-size": 5,
            "line": {
        ...
        this._xLines = [];
        for (var y = 1; y < this._config["step-size"]; ++y) {
            this._xLines.push(this._cross.line(0, 0, 0, 0));
        }
        this._yLines = [];
        for (var x = 1; x < this._config["step-size"]; ++x) {
            this._yLines.push(this._cross.line(0, 0, 0, 0));
        }
        this._xLines.unshift(this._cross.line(0, 0, 0, 0));
        this._yLines.unshift(this._cross.line(0, 0, 0, 0));
    });
