...
dudeGraph.Renderer.prototype.initialize = function (cgGraph, svgElement, config) {
    ...
    this._graph.on("cg-point-create", function (block, point) {
        if (block instanceof dudeGraph.Expression) {
            var renderBlocks = renderer.renderBlocksByBlock(block);
            _.forEach(renderBlocks, function (renderBlock) {
                renderer.createRenderPoint({
                    "renderBlock": renderBlock,
                    "point": point,
                    "index": point.isOutput ? renderBlock.renderOutputPoints.length : renderBlock.renderInputPoints.length
                }, true);
            });
        }
    });
