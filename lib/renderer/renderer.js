...
dudeGraph.Renderer = function () {
     ...
     * @type {Object<String, dudeGraph.RenderBlock>}
     * @private
     */
    this._renderBlockTypes = null;

    /**
     * The renderPoint types
     * @type {Object<String, dudeGraph.RenderPoint>}
     * @private
     */
    this._renderPointTypes = null;
     ...
     * @type {Object<String, dudeGraph.RenderGroup>}
     * @private
     */
    ...
    Object.defineProperty(this, "d3Blocks", {
        get: function () {
            return this._d3Blocks.selectAll(".dude-graph-block");
        }.bind(this)
    });
 ...
 * @param {Object} [config]
 */
dudeGraph.Renderer.prototype.initialize = function (graph, svgElement, config) {
    var renderer = this;
    this._graph = graph;
    ...
    this._d3Blocks = this._d3Root.append("svg:g").attr("id", "dude-graph-blocks");
    ...
    this._renderPointTypes = {
        "Point": dudeGraph.RenderPoint
    };
    ...
    this._renderGroupsQuadtree = null;
    this._renderPointsQuadtree = null;
    this._config = _.defaultsDeep(config || {}, dudeGraph.defaultRendererConfig);
    this._zoom = dudeGraph.defaultRendererZoom;
    this._graph.on("block-remove", function (block) {
        var renderBlocks = renderer.renderBlocksByBlock(block);
        _.forEachRight(renderBlocks, function (renderBlock) {
            renderer.removeRenderBlock(renderBlock, true);
        });
    });
    this._graph.on("block-point-remove", function (block, point) {
        var renderBlocks = renderer.renderBlocksByBlock(block);
        _.forEach(renderBlocks, function (renderBlock) {
            var renderPoint = renderer.renderPointByName(renderBlock, point.pointName, point.pointOutput);
            renderer.removeRenderPoint(renderPoint, true);
            if (renderBlock.renderGroupParent !== null) {
                renderBlock.updateRenderGroupParent();
            }
        });
    });
    this._graph.on("block-point-add", function (block, point) {
        var renderBlocks = renderer.renderBlocksByBlock(block);
        _.forEach(renderBlocks, function (renderBlock) {
    ...
    this._graph.on("point-value-change", function (point) {
        var renderBlocks = renderer.renderBlocksByBlock(point.pointBlock);
        _.forEach(renderBlocks, function (renderBlock) {
            renderBlock.update();
        });
    });
...
/**
 * Registers a renderPoint
 * @param {String} renderPointType
 * @param {dudeGraph.RenderPoint} renderPointConstructor
 */
dudeGraph.Renderer.prototype.registerRenderPoint = function (renderPointType, renderPointConstructor) {
    this._renderPointTypes[renderPointType] = renderPointConstructor;
};

/**
 * @typedef {Object} dudeGraph.Renderer.rendererConfigTypedef
 */

/**
 * @typedef {Object} dudeGraph.Renderer.rendererZoomTypedef
