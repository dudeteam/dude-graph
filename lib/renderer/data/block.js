...
dudeGraph.Renderer.prototype.createRenderBlock = function (renderBlockData, forceUpdate) {
    var renderer = this;
    var block = this._graph.blockById(renderBlockData.block);
    if (block === null) {
    ...
    var createRenderPoint = function (point) {
        var renderPointCustomData = renderBlockData[point.pointOutput ? "outputs": "inputs"];
        renderPointCustomData = renderPointCustomData && renderPointCustomData[point.pointName];
        renderer.createRenderPoint(renderBlock, {
            "renderPoint": renderPointCustomData,
            "point": point
        });
    };
    _.forEach(renderBlock.block.blockOutputs, createRenderPoint);
    _.forEach(renderBlock.block.blockInputs, createRenderPoint);
    if (forceUpdate) {
        this.createD3Blocks();
    }
    return renderBlock;
...
dudeGraph.Renderer.prototype.removeRenderBlock = function (renderBlock, forceUpdate) {
    ...
    if (_.isEmpty(this.renderBlocksByBlock(renderBlock.block))) {
        if (this._graph.blockById(renderBlock.block.blockId) !== null) {
