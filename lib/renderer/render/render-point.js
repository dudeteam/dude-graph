...
cg.Renderer.prototype._createPoints = function (points) {
    ...
    pointGroup
        .call(d3.behavior.drag()
            ...
            .on("dragend", function () {
                var connection = renderer._cursorConnection;
                var cursorPoint = renderer._cursorPoint;
                //var newPoint = renderer._getNearestPointInArea(cursorPoint.position, 10);
                var newPoint = null;
                renderer._cursorPoint = null;
                renderer._cursorConnection = null;
                if (newPoint !== null) {
                    console.log("Link connection !");
                    //connection.samePoint(cursorPoint, otherPoint);
                    renderer._graph.addConnection();
                } else {
                    console.log("Try to add a new block, then link...");
                }
                renderer._renderConnections();
            })
        )
