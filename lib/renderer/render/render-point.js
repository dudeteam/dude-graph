    ...
    points
        .each(function(point) {
            ...
            if (point.isInput && point.block.inputs.length > point.block.outputs.length) {
                point.block.data.minHeight += this.getBBox().height + renderer._config.point.padding;
...
cg.Renderer.prototype._getPointAbsolutePosition = function (point) {
    if (point instanceof cg.Point) {
        var actionPosition = point.block.absolutePosition;
        return new pandora.Vec2(
            actionPosition.x + (point.isInput ? this._config.block["padding"] : point.block.data.computedWidth - this._config.block["padding"]),
            actionPosition.y + point.block.data.computedHeadingOffset + point.index * this._config.point["height"]
        );
    }
