...
dudeGraph.Renderer.prototype._createD3Connections = function () {
    var createdD3Connections = this.d3Connections
        ...
        .enter()
        .append("svg:path");
    this._classD3Connections(createdD3Connections);
...
dudeGraph.Renderer.prototype._updateSelectedD3Connections = function (updatedD3Connections) {
    var renderer = this;
    this._classD3Connections(updatedD3Connections)
 ...
 * @param {d3.selection} d3Connections
 * @private
 */
dudeGraph.Renderer.prototype._classD3Connections = function (d3Connections) {
    d3Connections.attr("class", function (rendererConnection) {
        return [
            "dude-graph-connection",
            "dude-graph-output-" + _.kebabCase(rendererConnection.outputRendererPoint.cgPoint.cgValueType),
            "dude-graph-input-" + _.kebabCase(rendererConnection.inputRendererPoint.cgPoint.cgValueType)
        ].join(" ");
    });
    return d3Connections;
};