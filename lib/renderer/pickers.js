...
cg.pickerVec2 = function (element, value, pos, style) {
    ...
    group.add(cg.pickerString(group, value.x, pos, style));
    group.add(cg.pickerString(group, value.y, new cg.Vec2(pos.x + style.width + style.margin * 2, pos.y), style));
    return group;
};
...
cg.pickerVec3 = function (element, value, pos, style) {
    ...
    group.add(cg.pickerString(group, value.x, pos, style));
    group.add(cg.pickerString(group, value.y, new cg.Vec2(pos.x + style.width + style.margin * 2, pos.y), style));
    group.add(cg.pickerString(group, value.z, new cg.Vec2(pos.x + style.width * 2 + style.margin * 4, pos.y), style));
    return group;
};
