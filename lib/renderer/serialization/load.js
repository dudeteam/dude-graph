 ...
 * @param {{translate: [Number, Number], scale: Number}} data.zoom
 * @param {{translate: [Number, Number], scale: Number}} data.config.zoom
 */
dudeGraph.Renderer.prototype.load = function (data) {
    ...
    this._zoom = _.defaultsDeep((data.config && data.config.zoom) || data.zoom || {}, dudeGraph.Renderer.defaultZoom);
    this._updateZoom();
    this.createD3Blocks();
    this.createD3Groups();
