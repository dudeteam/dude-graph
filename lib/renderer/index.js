cg.Renderer = function () {
    ...
    function Renderer(paper) {
        ...
        paper.mousemove(function (e) {
            if (this._rectSelection) {
                var box = this._rectSelection.getBBox();
                var w = e.clientX - box.x;
                var h = e.clientY - box.y;
                if (w < 0) {
                    // TODO translate -w
                    this._rectSelection.attr("width", -w);
                } else {
                    this._rectSelection.attr("width", w);
                }
                if (h < 0) {
                    // TODO translate -h
                    this._rectSelection.attr("height", -h);
                } else {
                    this._rectSelection.attr("height", h);
                }
            }
        ...
        paper.mousedown(function (e) {
            if (e.shiftKey) {
                this._rectSelection = this._rootElement.rect(e.x, e.y, 0, 0);
    ...
    Renderer.prototype._renderGraph = function (graph, element) {
        ...
        this.render(graph.connections, element);
        this.render(graph.actions, element);
    };
