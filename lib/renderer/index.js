cg.Renderer = (function () {
     ...
     * @param width {Number?}
     * @param height {Number?}
     * @param handleSelection {Boolean?} create a group from selection, put all selected nodes inside the group
     * @return {cg.Group}
     */
    Renderer.prototype.createGroup = function (id, name, width, height, handleSelection) {
        var position = this._mousePosition.clone();
        var size = new pandora.Vec2(width || this._config.group["default-width"], height || this._config.group["default-height"]);
        if ((handleSelection || true) && this.selectedElements().length !== 0) {
            var selectedNodesBBox = this.selectedElements().getBBox();
            position = new pandora.Vec2(selectedNodesBBox.x - this._config.group.padding, selectedNodesBBox.y - this._config.group.padding - this._config.group["heading"]);
            size = new pandora.Vec2(selectedNodesBBox.width + this._config.group.padding * 2, selectedNodesBBox.height + this._config.group.padding * 2 + this._config.group["heading"]);
        }
