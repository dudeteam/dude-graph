cg.Renderer = function () {
    ...
    Renderer.prototype._renderConnection = function (connection, element) {
        ...
        path.mousedown(function (e, x, y) {
            if (path.hasClass("selected")) {
                path.removeClass("selected");
            } else {
                path.addClass("selected");
            }
            this._rootElement.selectAll('#connections > path').forEach(function (connection) {
                if (!e.shiftKey && path !== connection) {
                    connection.removeClass("selected");
                }
                connection.attr("strokeWidth", connection.hasClass('selected') ? 3 : 2);
            });
            cg.preventCallback(x, y, e);
        }.bind(this));
        return path;
