cg.Renderer = (function () {
    ...
    /**
     * Create group from selection if elements are selected, or default empty group
     * @param id {Number|String}
     * @param name {String}
     * @return {cg.Group}
     */
    Renderer.prototype.createEmptyGroup = function (id, name) {
        var position = new pandora.Vec2(this._config.group.padding, this._config.group.padding - this._config.group["heading"]);
        var size = new pandora.Vec2(this._config.group["default-width"] + this._config.group.padding * 2, this._config.group["default-height"] + this._config.group.padding * 2 + this._config.group["heading"]);
        var group = new cg.Group(id, name, position, size);
        this._graph.addNode(group, this._graph);
        return group;
    };
     ...
     * @return {cg.Group}
     */
    Renderer.prototype.createGroup = function (id, name) {
        if (this.selectedElements(true).length) {
