cg.Renderer = function () {
    ...
    function Renderer(paper) {
        this._paper = paper;
        var tmp = paper.g();
        this._rootElement = tmp.g();
        var foreground = tmp.g();
        //cg.createElement('<input type="text">', 20, 20, 50, 50, this._rootElement);
        cg.pickerVec3(foreground, {x: 1, y: 32, z: -5}, new cg.Vec2(170, 332));
        cg.pickerVec2(foreground, {x: 0.234, y: 0.4}, new cg.Vec2(140, 352));
        cg.pickerString(foreground, "a bigger text", new cg.Vec2(155, 372));
        cg.pickerBoolean(foreground, true, new cg.Vec2(170, 395));
    ...
    Renderer.prototype._renderAction = function (action, element) {
        ...
        actionGroup.drag(
            function move(dx, dy) {
                ...
                var actionGroupBBox = actionGroup.getBBox();
                action.position.x = actionGroupBBox.x;
                action.position.y = actionGroupBBox.y;
                action.emit("move");
            }.bind(this),
