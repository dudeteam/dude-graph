...
cg.Renderer.prototype._renderGraph = function (graph, element) {
    ...
    // this.render(graph.children, element); // Call this to render the graph in one-shot without using event emissions.
...
cg.Renderer.prototype._applyDeselection = function (e) {
    this._paper.selectAll(".action, .group").forEach(function (element) {
...
cg.Renderer.prototype._applyRectangleSelection = function (e) {
    if (this._selectionRectangle) {
        ...
        this._paper.selectAll(".block, .group").forEach(function (element) {
