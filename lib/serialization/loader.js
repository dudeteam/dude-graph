cg.Loader = (function () {
    ...
    Loader.prototype.load = function (graph, data) {
        for (var dataName in data) {
            if (data.hasOwnProperty(dataName)) {
                var loader = this[cg.camelize("_load " + dataName)];
    ...
    Loader.prototype._loadChildren = function (modelsData, graph) {
        for (var childIndex = 0; childIndex < modelsData.length; ++childIndex) {
            ...
            graph.addChild(this.loadNode(childData, graph));
        }
    };
     ...
     * @returns {cg.Node}
     */
    Loader.prototype.loadNode = function (nodeData, graph) {
        ...
        var nodeLoader = this[cg.camelize("_load node " + nodeData._type)];
        if (nodeLoader) {
            return nodeLoader.call(this, nodeData, graph);
        } else {
     ...
     * @private
     */
    Loader.prototype._loadNodeAction = function (actionData, graph) {
        var action = new cg.Action(actionData._id, graph, graph.getModel(actionData.name), new cg.Vec2(actionData.position));
     ...
     * @private
     */
    Loader.prototype._loadNodeGroup = function (groupData, graph) {
        var group = new cg.Group(groupData._id, graph, groupData.name, new cg.Vec2(groupData.position));

        for (var childIndex = 0; childIndex < groupData.children.length; ++childIndex) {
            var childData = groupData.children[childIndex];
            group.addChild(this.loadNode(childData, graph));
        }
        return group;
    };
