cg.JSONLoader = (function () {
    ...
    JSONLoader.prototype._loadModels = function (modelsData, graph) {
        for (var modelName in modelsData) {
            if (modelsData.hasOwnProperty(modelName)) {
                ...
                } else if (modelData.type === "getter") {
                    graph.addModel(new cg.Getter(modelName, modelData["value-type"]));
                }
            }
        }
    };
     ...
     * @private
     */
    JSONLoader.prototype._loadNodeAction = function (actionData, graph, parent) {
