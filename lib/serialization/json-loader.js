cg.JSONLoader = (function () {
    ...
    JSONLoader.prototype._loadModels = function (modelsData, graph) {
        for (var modelName in modelsData) {
            if (modelsData.hasOwnProperty(modelName)) {
                ...
                graph.addModel(new cg.Model(modelName, modelData.inputs, modelData.outputs, modelData.type));
            }
        }
    };
     ...
     * @returns {cg.Node}
     */
    JSONLoader.prototype.loadNode = function (nodeData, graph) {
     ...
     * @private
     */
    JSONLoader.prototype._loadNodeGroup = function (groupData, graph) {
     ...
     * @private
     */
    JSONLoader.prototype._loadNodeAction = function (actionData, graph) {
        var action = new cg.Action(actionData._id, graph.getModel(actionData.name), new cg.Vec2(actionData.position));
