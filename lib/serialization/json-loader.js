cg.JSONLoader = (function () {
    ...
    /**
     * Loads the given graph from the given data.
     * @param cgGraph {cg.Graph} A reference on the graph to load
     * @param graphData {{connections: Array, blocks: Array}} the json data used to generate the graph.
     */
    JSONLoader.prototype.load = function (cgGraph, graphData) {
        this._loadBlocks(cgGraph, graphData.blocks);
        this._loadPoints(cgGraph, graphData.blocks);
        if (graphData.connections) {
            this._loadConnections(cgGraph, graphData.connections);
        }
    };
